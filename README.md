...
- **Reminders:** optional Resend emails three days, one day, and on the due day using the candidate’s calendar date; duplicate deliveries are prevented. Hobby deployments use Vercel’s once-daily UTC cron cadence.
...
5. Open the six-language assessment and run a short JavaScript or Python solution through server-evaluated checks.
...
Job-board DOM → Chrome extension → paired extension key → OpenRouter + Exa analysis
     ...
     Supabase (GitHub auth, RLS data, approved evidence, private resume storage)
                                                           ↓
    Assessment proxy → Judge0 sandbox | Voice → OpenRouter STT/LLM/TTS | Email → Resend
```
...
- Judge0 CE is suitable only for low-volume practice. A public launch should use a dedicated Judge0 deployment or a managed sandbox with server-only credentials.
